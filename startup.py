...
def AddToRegistry():
    ...
    s_name = r"testscript.py" # ".\\testscript.py"
    address = os.path.join(path, s_name)

    key = reg.HKEY_CURRENT_USER
    key_value = "Software\Microsoft\Windows\CurrentVersion\Run" # HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Run

    open = reg.OpenKey(key, key_value, 0, reg.KEY_ALL_ACCESS)
    reg.SetValueEx(open, "testscript", 0, reg.REG_SZ, address)
    reg.CloseKey(open)
...
def RemoveFromRegistry():
    ...
    open = reg.OpenKey(key, key_value, 0, reg.KEY_ALL_ACCESS)
    reg.DeleteValue(open, "testscript")
    reg.CloseKey(open)
